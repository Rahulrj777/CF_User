...
import { api } from "../../api.js"; // centralized backend
...
const Acting = () => {
  ...
  // Fetch acting mentors
  useEffect(() => {
    const fetchMentors = async () => {
      try {
        const res = await api.get("/actingmentor");
        setMentors(res.data);
      } catch (err) {
  ...
  // Fetch acting diploma content
  useEffect(() => {
    const fetchDiploma = async () => {
      try {
        const res = await api.get("/actingdiploma");
  ...
  const topPage = () => window.scroll(0, 0);
      ...
      <section className="border-t-4 border-orange-500 pt-10 pb-16 md:pt-20 md:pb-20 bg-gray-950 -mt-6">
        <div className="w-full px-4 md:w-[80%] mx-auto">
          ...
          <div className="flex justify-center items-center font-[poppins]">
            <div className="grid grid-cols-1 gap-y-8 md:grid-cols-2 md:gap-x-60 w-full">
              {contents.map((month, index) => (
                <div key={month._id || index} className="flex flex-col gap-y-6">
                      ...
                      <div>
                        <ul className="text-[13px] md:text-[14px] font-[roboto] flex flex-col gap-y-4 text-gray-200">
                          {month.children?.map((child, idx) => (
                            <li key={idx} className="flex items-center gap-x-3 md:gap-x-5">
                              <span>
          ...
          <div className="flex justify-center items-center mt-8 md:mt-20 font-[poppins]">
            {globalPdf ? (
              <a href={globalPdf.startsWith("http") ? globalPdf : `${api.defaults.baseURL}${globalPdf}`} target="_blank" rel="noopener noreferrer">
        ...
        <div className="px-4 w-full md:w-[80%] mx-auto font-kumbh">
          ...
          <div className="flex justify-center items-center">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 md:gap-y-16 gap-x-20">
              {mentors.map((mentor) => (
                <div key={mentor._id} className="flex flex-col items-center justify-center">
                  <div className="flex justify-center items-center">
                    <img
                      src={mentor.url}
